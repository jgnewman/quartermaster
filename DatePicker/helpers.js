...
export function nullableDateFromValue(value) {
    return value ? new Date(value) : null;
}
export function updatedDateFromValue(curItem, newNum) {
    if (!curItem) {
...
export function getDayTotalForMonth(month, fullYear) {
    if (month === 1) {
...
export function getLocalizedDay(date, weekStartsOnMonday) {
    let day = date.getDay();
...
export function setDateToNextIncrement(date, increment, from) {
    if (from && from > date) {
...
export function setDateToLastMS(date) {
    date.setHours(23);
    date.setMinutes(59);
    date.setSeconds(59);
    date.setMilliseconds(999);
}
export function getCalendarData(disablePast, referenceMonth, referenceYear, weekStartsOnMonday) {
    const now = new Date();
...
export function isEarlierDayThan(a, b) {
    if (a === null || b === null) {
        return false;
    }
    if (b < a) {
...
export function getTimeMapFromDate(date, disablePast, isEndDate, now, timeIncrement) {
